...
const HeroCarousel = () => {
  ...
  // Define drag threshold (10% of screen width for a valid swipe)
  ...
  const getClientX = (e) => {
      // FIX: Safely access clientX from the nativeEvent if possible
      const event = e.nativeEvent || e;
      // Use clientX for mouse events, or the first touch point's clientX for touch events
      return event.clientX || (event.touches && event.touches[0].clientX) || (event.changedTouches && event.changedTouches[0].clientX) || 0;
  };
  ...
  const handleDragEnd = (e) => {
    // Only proceed if dragging was actively initiated
    if (!isDragging) return;
    setIsDragging(false);

    // Get ending X position from mouse or touch event
    const dragEndX = getClientX(e); // Pass the React SyntheticEvent
  ...
  // Prevent vertical scrolling interference on touch devices
  const handleMove = (e) => {
    // Check if it's a touch event and prevent default scrolling
